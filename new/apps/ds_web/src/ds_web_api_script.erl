-module(ds_web_api_script).

-behaviour(ds_web_api_behaviour).

-export([
         get_sub_handler/3,
         delete/1,
         forbidden/2,
         create/2,
         exists/1,
         list_resources/0,
         list_resources_for_parent/1,
         get_data/1,
         get_owner/1,
         put_data/2]).
%% REST

%% Init Section


%% Implementation

get_sub_handler(Parent, This, _Path) ->
    {Parent, ds_web_api_script, This}.

delete(Id) ->
    case pgapp:equery("DELETE FROM scripts WHERE id = $1", [Id]) of
        {ok, 1} ->
            true;
        {ok, 0} ->
            false;
        _Error ->
            false
    end.

forbidden(Id, UId) ->
    case pgapp:equery("SELECT user_id FROM scripts WHERE id = $1", [Id]) of
        {ok, _, [{UId}]} ->
            false;
        {ok, _, []} ->
            true
    end.

exists(Id) ->
    case pgapp:equery("SELECT count(*) FROM scripts WHERE id = $1", [Id]) of
        {ok, _, [{1}]} ->
            true;
        _ ->
            false
    end.

create(UId, [{user, UId}]) ->
    {ok, _, _, [{TypeId}]} =
        pgapp:equery("INSERT INTO scripts (user_id) VALUES ($1) RETURNING id", [UId]),
    Location = list_to_binary(io_lib:format("~p", [TypeId])),
    UIdStr = list_to_binary(io_lib:format("~p", [UId])),
    {<<"/api/v1/user/", UIdStr/binary, "/script/", Location/binary>>, TypeId}.

%%Internal


list_resources() ->
    {ok, _, SIds} =
        pgapp:equery("SELECT id, name FROM scripts"),
    List = lists:map(fun ({Id, Name}) ->
                             [{<<"id">>, Id},
                              {<<"name">>, Name}]
                     end, SIds),
    {ok, List}.

list_resources_for_parent([{user, UId}]) ->
    {ok, _, SIds} =
        pgapp:equery("SELECT id, name FROM scripts WHERE user_id = $1 OR user_id = 0", [UId]),
    List = lists:map(fun ({Id, Name}) ->
                             [{<<"id">>, Id},
                              {<<"name">>, Name}]
                     end, SIds),
    {ok ,List}.

get_owner(Id) ->
    {ok, _, [{Owner}]} =
        pgapp:equery("SELECT user_id FROM scripts where id = $1", [Id]),
    {ok, Owner}.

get_data(Id) ->
    {ok, get_obj(Id)}.

put_data(Id, Obj) ->
    {<<"user_id">>, UserId} = lists:keyfind(<<"user_id">>, 1, Obj),
    {<<"name">>, Name} = lists:keyfind(<<"name">>, 1, Obj),
    {<<"code">>, Code} = lists:keyfind(<<"code">>, 1, Obj),
    {ok, _, _, [{RespId, UserId, Name, Code}]} =
        pgapp:equery("UPDATE scripts SET user_id = $2, name = $3, code = $4" ++
                         "WHERE id = $1 RETURNING id, user_id, name, code", [Id, UserId, Name, Code]),
    {ok,
     [{<<"id">>, RespId},
      {<<"user_id">>, UserId},
      {<<"name">>, Name},
      {<<"code">>, Code}]}.

get_obj(Id) ->
    {ok, _, [{RespId, UserId, Name, Code}]} =
        pgapp:equery("SELECT id, user_id, name, code FROM scripts WHERE id = $1", [Id]),
    [{<<"id">>, RespId},
     {<<"user_id">>, UserId},
     {<<"name">>, Name},
     {<<"code">>, Code}].
